<div id="ajax_wrap">
  <div style="padding:10px">
    ...
    <div class="fright">
      <g:paginate controller="client" action="${actionName}" params="${params}" 
    ...
    <table class="list" width="100%" cellpadding="0" cellspacing="0" border="0">
      ...
      <g:each in="${history.records}" status="i" var="record">
        <tr align="center">          
          ...
          <td><g:if test="${record.is_fromclient}">я</g:if><g:else>admin</g:else></td>          
          ...
          <td><g:if test="${record.filename}"><a href="${createLink(controller:'client', action:'picture',params:[id:record.mbox_id,filename:record.filename])}" target="_blank"><i class="icon-paper-clip icon-large" title="${record.filename}"></i></a></g:if></td>
        </tr>
        <tr id="text${i}" style="color:#777">
          <td colspan="4" style="padding:8px 15px!important">
            <g:if test="${record.filename&&record.filename[-3..-1]=='jpg'}"><figure style="width:auto;float:left;padding-right:10px"><img src="${createLink(controller:'client', action:'picture',params:[id:record.mbox_id,filename:'t_'+record.filename])}" alt="" /></figure></g:if>
