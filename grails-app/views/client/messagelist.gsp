...
<g:if test="${messages.records}">
  <div id="resultList">
    <table class="list" width="100%" cellpadding="0" cellspacing="0" border="0">
      ...
      <tbody>
      <g:each in="${messages.records}" status="i" var="record">
        <tr align="center" <g:if test="${!record.is_clientread}">class="noread"</g:if>>
          <td onclick="starToggle(${record.id})"><i id="star${record.id}" class="icon-star${!record.is_clientfavourite?'-empty':''} icon-gold icon-large"></i></td>
          <td>${record.id?:0}</td>
          <td>${record.is_fromclient?'admin':'мне'}</td>          
          ...
          <td>${record.nrec?:0}</td>          
          <td><i class="icon-${record.modstatus==1?'envelope'+(record.is_clientread?'-alt':''):'trash'}"></i></td>
          <td>
